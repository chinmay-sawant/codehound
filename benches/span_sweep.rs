...
fn nested_findings_and_spans(count: usize) -> (Vec<Finding>, Vec<FunctionSpan>) {
    let mut findings = Vec::with_capacity(count);
    let mut spans = Vec::with_capacity(count);
    for index in 0..count {
        spans.push(FunctionSpan {
            start_byte: index,
            end_byte: count * 100 + index,
            start_line: 1,
            end_line: count + 10 - index,
            depth: index,
        });
        findings.push(Finding::new(FindingInputs::new(
            "PERF-BENCH-NESTED",
            "nested benchmark finding",
            "bench.go",
            LineCol::try_new(2, 1).expect("valid benchmark location"),
            "benchmark",
            Severity::Info,
            std::borrow::Cow::Borrowed(&[]),
        )));
...
fn bench_span_sweep(c: &mut Criterion) {
    ...
    for count in [100, 1_000] {
        let (template, spans) = nested_findings_and_spans(count);
        c.bench_function(&format!("span_sweep_nested_{count}"), |b| {
            b.iter_batched(
                || template.clone(),
                |mut findings| {
                    bench_attach_function_context(&mut findings, black_box(&spans));
