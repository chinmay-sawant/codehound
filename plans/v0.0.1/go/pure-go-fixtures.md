...
- [~] Pre-existing off-by-one in `tests/fixtures/go/` ↔ manifest resolved (needs review) (deferred → see plans/v3.0.0/)
- [~] `cargo test --test go_integration` passes (needs review) (deferred → see plans/v3.0.0/)
- [~] `cargo test --test fixture_manifest_integration` passes (needs review) (deferred → see plans/v3.0.0/)
