...
| **P2** | Flatten 88 `mod.rs` Go detector tree | ❌ | ❌ Open | Deferred to `plans/v0.0.2/restructure-codebase/` |
...
3. Execute Go detector flatten per `plans/v0.0.2/restructure-codebase/`.
...
- [~] Shrink `engine/mod.rs` `pub use` surface (prelude exists; full narrow deferred) — (needs review: prelude exists, 13+ groups in mod.rs remain) (deferred → see plans/v0.0.3/)
