# v0.0.2 — Rust Remediation Phase 3
...
- [~] `cargo fmt --check` — (needs review: still fail, formatting drift) (deferred → see plans/v0.0.3/)
...
- [~] `scan_entry` orchestrator — **76 lines** (target <60; needs further trimming) (deferred → see plans/v0.0.3/)
...
- [~] Replace or document 3 production `.expect(` (restore `check_no_prod_expect.sh` green) (deferred → see plans/v0.0.3/)
- [x] Rebaseline or optimize index build for `perf_regression` smoke budget (deferred → see plans/v0.0.3/) (now implemented)
- [~] `cargo fmt` + commit formatting (deferred → see plans/v0.0.3/)
...
- [~] Trim `scan_entry` orchestrator to <60 lines (deferred → see plans/v0.0.3/)
