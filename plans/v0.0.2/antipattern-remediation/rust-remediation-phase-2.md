# v0.0.2 — Rust Remediation Phase 2

> **Parent:** `plans/v0.0.2/rust-best-practices.md`, `m15-anti-pattern.md`, `rust-patterns.md`
...
- [x] `Registry::default` / builder methods — (needs review: deferred, `#[must_use]` not added) (deferred → see plans/v0.0.3/) (now implemented)
...
- [~] Document heuristic-only rules in registry TOML comment — (needs review: not confirmed) (deferred → see plans/v0.0.3/)
...
- [~] Enable `#![warn(missing_docs)]` on `src/lib.rs` — (needs review: still deferred; only on `rules/mod.rs`) (deferred → see plans/v0.0.3/)
...
- [~] Runnable doc-test for `lib.rs` quick-start — (needs review: still `#no_run`) (deferred → see plans/v0.0.3/)
- [~] `#![deny(missing_docs)]` ratchet on one module — (needs review: `warn` on `rules/mod.rs` only) (deferred → see plans/v0.0.3/)
...
- [~] `cargo insta test` CI step — (needs review: not configured) (deferred → see plans/v0.0.3/)
- [~] `pretty_assertions` still unused — (needs review: dep in Cargo.toml, 0 usages) (deferred → see plans/v0.0.3/)

### 2C.3 Test structure (Chapter 5)

- [~] Split multi-assert envelope tests — (needs review: still deferred) (deferred → see plans/v0.0.3/)
- [~] Split SARIF log tests — (needs review: still deferred) (deferred → see plans/v0.0.3/)
- [~] Naming convention audit — (needs review: still deferred) (deferred → see plans/v0.0.3/)
...
- [~] Deprecate direct `engine::*` re-exports — (needs review: not started) (deferred → see plans/v0.0.3/)
- [x] `#[cfg(feature = "cli")]` on `pub mod cli` in `lib.rs` — confirmed
- [x] Update `src/main.rs` to use `codehound::cli` via feature gate — (needs review: not confirmed) (deferred → see plans/v0.0.3/) (now implemented)
...
- [x] Execute `plans/v0.0.2/restructure-codebase/` — all 6 phases complete per `inventory.md`
- [~] ~~Newtype `RuleId` / `FilePath` on `Finding`~~ (removed by ponytail cleanup: thin newtypes deleted)
- [x] Type-state `AnalyzerBuilder<HasRegistry, HasFilter>` — (needs review: simple builder, no type-state) (deferred → see plans/v0.0.3/) (now implemented)
