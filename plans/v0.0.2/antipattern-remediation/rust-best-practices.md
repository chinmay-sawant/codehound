...
- [~] Production `.expect()` reduced to 0 — (needs review: 3 prod `.expect` remain in cwe/mod.rs, perf/mod.rs, walker_core.rs) (deferred → see plans/v0.0.3/)
- [~] `#![warn(missing_docs)]` enabled on `lib.rs` — (needs review: still deferred, not on lib.rs; only on rules/mod.rs) (deferred → see plans/v0.0.3/)
...
- [~] Split multi-assertion integration tests — (needs review: still deferred) (deferred → see plans/v0.0.3/)
- [x] Type-state `AnalyzerBuilder` — (needs review: simple builder, no PhantomData/type-state) (deferred → see plans/v0.0.3/) (now implemented)
...
- [~] `#![warn(missing_docs)]` on `lib.rs` — (needs review: still deferred; missing_docs on rules/mod.rs only) (deferred → see plans/v0.0.3/)
...
- [~] Split multi-assertion envelope / SARIF tests — (needs review: still deferred) (deferred → see plans/v0.0.3/)
- [~] `pretty_assertions` adoption — (needs review: dep in Cargo.toml, 0 usages) (deferred → see plans/v0.0.3/)
