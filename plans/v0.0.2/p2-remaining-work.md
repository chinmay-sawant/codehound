...
> **Detailed breakdown:** See `plans/v0.0.2/pending-work/` for individual per-workstream plans.
...
- [x] **Phase C — Remove substring fallback for CWE-78/89/22/79** (completed — substring fallback removed, taint enabled by default)
  - 📋 Detailed plan: `plans/v0.0.2/pending-work/01-taint-tracking-remaining.md` Phase C
- [x] **Phase D — Extended sanitizer coverage** — `strconv.Atoi`, `html.EscapeString`, and name-based heuristic sanitizers added; `utf8.ValidString`/`net/url.IsAbs`/`strings.HasPrefix` skipped (return bools)
  - 📋 Detailed plan: `plans/v0.0.2/pending-work/01-taint-tracking-remaining.md` Phase D
- [x] **Phase E — CLI `--show-taint` / `--taint` / `--no-taint` + documentation** — flags added, `documents/taint.md` created, taint enabled by default
  - 📋 Detailed plan: `plans/v0.0.2/pending-work/01-taint-tracking-remaining.md` Phase E + `plans/v0.0.2/pending-work/05-cross-cutting-remaining.md` Phase 2.1
- [x] **Phase F — Inter-procedural taint** (completed — call graph, function summaries, cross-function propagation, evidence, fixtures all shipped in 6 phases)
  - 📋 Detailed plan: `plans/v0.0.2/pending-work/01-taint-tracking-remaining.md` Phase F

---

## A. P2.3 — Incremental Analysis

📋 Detailed plan: `plans/v0.0.2/pending-work/04-cache-incremental-remaining.md`

### A.1 Plan items still unchecked in `03-incremental-analysis.md`

- [~] **Phase 4.2 — Store inline-ignore set inside the cache entry** (deferred → see plans/v0.0.3/)
...
- [~] **Phase 4.3 — Detect `dependencies`-list change on identical content hash** (deferred → see plans/v0.0.3/)
...
📋 Detailed plan: `plans/v0.0.2/pending-work/02-perf-detectors-remaining.md`
...
- [~] **Phase 5 — Performance verification** (deferred → see plans/v0.0.3/)
...
- [x] **Batch 7 (22):** PERF-138, 159, 167, 169, 173, 174, 175, 178, 179, 180, 183, 184, 185, 186, 187, 188, 193, 194, 202, 207, 210, 212. Migrated from `hot_path_misc.rs` to domain modules. See `plans/v0.0.2/reports/domain-migration-review.md`.
...
📋 Detailed plan: `plans/v0.0.2/pending-work/03-bad-practices-remaining.md`
...
- [~] **Add per-rule severity overrides** — like the PERF metadata overrides, allow `[bad_practices.severity_overrides]` in the config to bump a single rule (e.g. BP-5 → High) without touching the global severity. (deferred → see plans/v0.0.3/)
...
- [~] **HTML reporter** — render BP findings with the same color band as the text reporter (deferred until the HTML reporter is added at all; today only text/JSON/SARIF exist). (deferred → see plans/v0.0.3/)
...
- [~] **BP-15 regression test** — current fixture only checks the recursive `sync.Once.Do` call; add a fixture where the closure is a separate function (the harder case to detect because the body has to walk up the call chain). (deferred → see plans/v0.0.3/)
...
📋 Detailed plan: `plans/v0.0.2/pending-work/05-cross-cutting-remaining.md`
...
- [~] **Add per-detector timing to the cache hit path** — the per-detector timing emitted today is only for files that get parsed. A cache hit should also report the saved parse+detect time per detector so the diagnostics document shows the win. (deferred → see plans/v0.0.3/)
