# v0.0.2 — Go Rule Autofix (`--fix`) Implementation Plan

> **Parent:** `ruleset/golang/golang.json` — authoritative CWE/PERF rule inventory; `plans/v0.0.2/pending-work/03-bad-practices-remaining.md` — BP inventory
...
- **No `--fix-unsafe` in v0.0.2.** Keep review-required candidates as
...
- [~] Remove the 112 nested `PERF-101`…`PERF-212` objects incorrectly stored as
  extra fields inside the top-level `PERF-100` object. (deferred → see plans/v0.0.3/)
...
- [~] Fix semantic mismatches before attaching edits. Known example:
  ...
  PERF-102 as HTTP transport sharing. (deferred → see plans/v0.0.3/)
- [~] Require each registered detector's fixture to assert the JSON rule name
  and intended pattern, not only the emitted ID. (deferred → see plans/v0.0.3/)
- [~] Verify all 175 CWE registry entries against their JSON descriptions. (deferred → see plans/v0.0.3/)
- [~] Move the 13 shipped BP definitions to the planned
  `ruleset/golang/bad-practices.json` before making BP fix metadata generated. (deferred → see plans/v0.0.3/)
...
- [~] Add a schema/coverage test that fails on extra fields, mixed ID formats,
  ...
  detector/message that does not match its rule definition. (deferred → see plans/v0.0.3/)
- [~] Run the registry check in `cargo test` and in CI. (deferred → see plans/v0.0.3/)
...
- [~] Keep `RuleMetadata.fix` and `Finding.fix` as human-readable remediation. (deferred → see plans/v0.0.3/)
...
- [~] Emit an edit only from an exact AST match. A detector's line/column,
  source-window heuristic, or metadata text is insufficient. (deferred → see plans/v0.0.3/)
- [~] Do not reuse the currently dormant `Finding.byte_offset` /
  ...
  replacement. Prefer an explicit `SourceEdit`. (deferred → see plans/v0.0.3/)
- [~] Keep fixes optional so reporters, caches, and non-Go detectors remain
  backward-compatible. (deferred → see plans/v0.0.3/)

### 1.2 Define the CLI contract

- [~] Add `--fix`: apply only `MachineApplicable` edits. (deferred → see plans/v0.0.3/)
- [~] Add `--fix-dry-run`: print the exact proposed diff and write nothing. (deferred → see plans/v0.0.3/)
- [~] Honor `--only`, `--skip`, `--bp-only`, `--no-bp`, path filters, ignored
  findings, and configuration severity/rule filters before planning edits. (deferred → see plans/v0.0.3/)
- [~] Make fix modes incompatible with `--baseline`, `--list-rules`,
  `--explain`, and cache-pruning commands. (deferred → see plans/v0.0.3/)
- [~] Force a fresh uncached scan in v0.0.2 fix mode. This is smaller and safer
  than changing the cache schema before the edit model stabilizes. (deferred → see plans/v0.0.3/)
- [~] Report files changed, edits applied, edits skipped, conflicts, formatter
  failures, and findings remaining after the verification scan. (deferred → see plans/v0.0.3/)
- [~] Compute the process exit code from the post-fix scan; return non-zero for
  edit conflicts or validation/write failures. (deferred → see plans/v0.0.3/)

### 1.3 Plan and apply edits safely

- [~] Validate the whole-file source hash and each edit's `expected` text before
  applying anything. (deferred → see plans/v0.0.3/)
- [~] Deduplicate identical edits. (deferred → see plans/v0.0.3/)
- [~] Reject overlapping non-identical edits; skip the affected file and report
  both rule IDs. (deferred → see plans/v0.0.3/)
- [~] Sort accepted edits by descending byte offset so earlier replacements do
  not invalidate later offsets. (deferred → see plans/v0.0.3/)
- [~] Apply all edits in memory. (deferred → see plans/v0.0.3/)
- [~] Run `gofmt` on the in-memory result through stdin; do not require
  `goimports` or add a dependency. (deferred → see plans/v0.0.3/)
- [~] Reparse the formatted result with the existing Go tree-sitter parser. (deferred → see plans/v0.0.3/)
- [~] Reuse/generalize `engine::cache::io::write_atomic` for the final write
  instead of introducing a second atomic-write implementation. (deferred → see plans/v0.0.3/)
- [~] Invalidate cache entries for changed files. (deferred → see plans/v0.0.3/)
- [~] Rescan changed files and fail the operation if an applied rule still
  reports the same finding. (deferred → see plans/v0.0.3/)

### 1.4 Keep import edits boring

- [~] Support only deterministic import insertion/removal inside the existing
  import block. (deferred → see plans/v0.0.3/)
- [~] Detect aliases, dot imports, blank imports, local identifiers that shadow
  package names, and multiple import blocks. (deferred → see plans/v0.0.3/)
- [~] Decline the edit when import resolution is ambiguous. (deferred → see plans/v0.0.3/)
- [~] Let `gofmt` format the result; do not implement a general Go pretty
  printer. (deferred → see plans/v0.0.3/)
...
- [~] **PERF-001 — Regular Expression Compilation Inside Loop:** hoist only a
  valid constant `regexp.MustCompile` pattern to a collision-free binding. (deferred → see plans/v0.0.3/)
- [~] **PERF-016 — Buffer Reallocation Inside Loop:** hoist a non-escaping
  `bytes.Buffer` and call `Reset`; refuse aliases, copies, or retained results. (deferred → see plans/v0.0.3/)
- [~] **PERF-024 — Crypto Hashing In Tight Loop:** reuse a non-escaping
  fixed-algorithm hasher with `Reset`; exclude keyed/dynamic HMAC construction. (deferred → see plans/v0.0.3/)
- [~] **PERF-044 — Repeated Type Assertion On Same Interface:** bind one
  ...
  reassigned. (deferred → see plans/v0.0.3/)
- [~] **PERF-096 — gRPC Stream Allocation Per Recv:** reuse and reset one
  ...
  iteration. (deferred → see plans/v0.0.3/)
- [~] **PERF-109 — Map Key Recomputed In Loop Without Caching:** hoist a pure,
  loop-invariant, non-panicking key expression. (deferred → see plans/v0.0.3/)
- [~] **PERF-141 — URL.Query() Called Repeatedly Without Caching:** cache one
  ...
  uses. (deferred → see plans/v0.0.3/)
- [~] **PERF-153 — http.Cookie.String Called Repeatedly:** cache the string only
  when the cookie and all aliases are not mutated. (deferred → see plans/v0.0.3/)
- [~] **PERF-179 — strings.Replacer Not Used For Repeated Replace:** hoist a
  replacer for loop-invariant replace-all operands. (deferred → see plans/v0.0.3/)
- [~] **PERF-192 — Map Without Size Hint:** add `len(src)` only for a proven
  one-to-one population loop with a known source collection. (deferred → see plans/v0.0.3/)
- [~] **PERF-203 — net.IP.String Repeated In Hot Path:** cache the result only
  when the IP slice and aliases are not mutated. (deferred → see plans/v0.0.3/)
...
- [~] **PERF-042 — fmt.Errorf With No Formatting In Hot Path:** replace a
  ...
  import repair. (deferred → see plans/v0.0.3/)
- [~] **PERF-111 — Range Over String Produces Rune Allocation:** replace
  ...
  absent or unused. (deferred → see plans/v0.0.3/)
- [~] **PERF-115 — strings.Compare Used For Equality Check:** replace exact
  zero equality/inequality with `==` / `!=`. (deferred → see plans/v0.0.3/)
- [~] **PERF-116 — strings.Index Used For Contains Check:** replace exact
  `-1` checks with `strings.Contains` or its negation. (deferred → see plans/v0.0.3/)
- [~] **PERF-117 — bytes.Compare Used For Equality Check:** replace exact zero
  equality/inequality with `bytes.Equal` or its negation. (deferred → see plans/v0.0.3/)
- [~] **PERF-120 — time.Now().Sub Instead Of time.Since:** replace the exact
  fresh-`Now` expression with `time.Since`. (deferred → see plans/v0.0.3/)
- [~] **PERF-124 — strings.Replace With -1 Instead Of ReplaceAll:** replace only
  when the count is the integer constant `-1`. (deferred → see plans/v0.0.3/)
- [~] **PERF-126 — Redundant http.CanonicalHeaderKey Call:** remove the call only
  for a literal already equal to its canonical form. (deferred → see plans/v0.0.3/)
- [~] **PERF-127 — Unnecessary fmt.Sprintf In Log Call:** remove a nested
  one-literal, no-directive `Sprintf` from a resolved standard logger call. (deferred → see plans/v0.0.3/)
- [~] **PERF-146 — fmt.Sprintf With Single String And No Verbs:** replace an
  exact one-string call with the string expression and repair imports. (deferred → see plans/v0.0.3/)
- [~] **PERF-147 — strings.Replace Call Where ReplaceAll Suffices:** use
  `ReplaceAll` only for exact replace-all semantics. (deferred → see plans/v0.0.3/)
- [~] **PERF-178 — time.Format Instead Of time.AppendFormat:** rewrite
  `append(dst, t.Format(layout)...)` to `t.AppendFormat(dst, layout)`. (deferred → see plans/v0.0.3/)
...
- [~] **PERF-103 — HTTP Response Body Not Closed:** insert
  ...
  guard when response ownership does not transfer. (deferred → see plans/v0.0.3/)
- [~] **PERF-113 — Single-Case Select Statement Instead Of Channel Op:** replace
  ...
  scope. (deferred → see plans/v0.0.3/)
- [~] **PERF-114 — Manual Loop Copy Instead Of copy() Builtin:** replace only the
  canonical side-effect-free full-copy loop with proven compatible bounds. (deferred → see plans/v0.0.3/)
- [~] **PERF-119 — Multiple Separate Appends Instead Of Spread Concatenation:**
  ...
  not observe or alias the destination slice. (deferred → see plans/v0.0.3/)
- [~] **PERF-121 — Struct Literal Instead Of Direct Type Conversion:** use a
  ...
  equivalence. (deferred → see plans/v0.0.3/)
- [~] **PERF-122 — HasPrefix Followed By Slice Instead Of TrimPrefix:** collapse
  an exact one-statement/no-else guard with stable expressions. (deferred → see plans/v0.0.3/)
- [~] **PERF-123 — Redundant make Argument With Zero Value:** remove zero only
  from proven map/channel forms; never rewrite `make([]T, 0, cap)`. (deferred → see plans/v0.0.3/)
- [~] **PERF-128 — Multiple Independent Appends Can Be Combined:** use the same
  purity/alias gates as PERF-119 for three or more appends. (deferred → see plans/v0.0.3/)
- [~] **PERF-129 — Range Loop Copies Value When Only Index Needed:** remove only
  an explicit blank value binding (`for i, _ := range`). (deferred → see plans/v0.0.3/)
- [~] **PERF-130 — Unnecessary Function Wrapper Adding Call Overhead:** inline
  ...
  call; exclude `go`, `defer`, return values, locals, and control flow. (deferred → see plans/v0.0.3/)
- [~] **PERF-133 — sort.Slice Closure Allocation Inside Loop:** replace only an
  exact ascending comparator over a proven basic slice type. (deferred → see plans/v0.0.3/)
- [~] **PERF-158 — Sorting Slice Of Basic Types With Closure:** share the
  PERF-133 fixer; explicitly gate float/NaN behavior and Go-version API choice. (deferred → see plans/v0.0.3/)
- [~] **PERF-167 — WaitGroup.Add Inside Goroutine:** move an unconditional
  positive-constant `Add` immediately before its matching goroutine. (deferred → see plans/v0.0.3/)
- [~] **PERF-173 — time.Tick Not Stopped Causing Goroutine Leak:** introduce a
  function-scoped `NewTicker`, use `.C`, and insert `defer Stop`. (deferred → see plans/v0.0.3/)
...
- [~] **BP-6 — WaitGroup Add Inside Goroutine:** reuse the PERF-167 fixer after
  ...
  implementations. (deferred → see plans/v0.0.3/)
...
- [~] **Loop allocation/reuse proposals:** PERF-002, PERF-003, PERF-004,
  ...
  freshness, error, capacity, Go-version, or user-limit decisions. (deferred → see plans/v0.0.3/)
- [~] **Gin proposals:** PERF-060, PERF-063, PERF-064, PERF-067, PERF-068,
  ...
  policy. (deferred → see plans/v0.0.3/)
- [~] **Database/framework proposals:** PERF-071, PERF-072, PERF-073, PERF-074,
  ...
  query, transaction, hook, batch, framework lifetime, or cache policy. (deferred → see plans/v0.0.3/)
- [~] **HTTP/runtime/structural proposals:** PERF-101, PERF-102, PERF-106,
  ...
  Unicode/protocol differences. (deferred → see plans/v0.0.3/)
- [~] **Database/concurrency/I/O proposals:** PERF-161, PERF-163, PERF-164,
  ...
  contract change. (deferred → see plans/v0.0.3/)
...
- [~] **BP-2:** wrap `return err` with inferred operation context only after
  reviewing error text/identity compatibility. (deferred → see plans/v0.0.3/)
- [~] **BP-4:** capture and report recovered panics using the project's logger
  and panic policy. (deferred → see plans/v0.0.3/)
- [~] **BP-5:** handle `Close` errors using a caller-approved return/log/merge
  strategy. (deferred → see plans/v0.0.3/)
- [~] **BP-7:** change mutex value parameters to pointers and update the closed
  caller set. (deferred → see plans/v0.0.3/)
- [~] **BP-10:** replace `time.After` loops with a correctly stopped, drained,
  and reset timer after reviewing lifecycle behavior. (deferred → see plans/v0.0.3/)
- [~] **BP-11:** scope per-iteration defers in a closure/helper only when
  `break`, `continue`, `return`, panic, and error semantics are preserved. (deferred → see plans/v0.0.3/)
- [~] **BP-13:** add and propagate a `context.Context` parameter across the
  complete caller set. (deferred → see plans/v0.0.3/)
...
- [~] Keep the existing vulnerable fixture and safe fixture for detection. (deferred → see plans/v0.0.3/)
- [~] Add exact transformed-source golden coverage. (deferred → see plans/v0.0.3/)
- [~] Add one refusal fixture for every safety precondition. (deferred → see plans/v0.0.3/)
- [~] Assert the transformed source parses and is already `gofmt`-clean. (deferred → see plans/v0.0.3/)
- [~] Assert the originating finding disappears on rescan. (deferred → see plans/v0.0.3/)
- [~] Assert a second fix run produces zero edits. (deferred → see plans/v0.0.3/)
- [~] Compile/run a temporary Go module where the rule's dependencies permit
  it; do not claim semantic safety from string snapshots alone. (deferred → see plans/v0.0.3/)

### 4.2 Edit-engine tests

- [~] Multiple non-overlapping edits in one file. (deferred → see plans/v0.0.3/)
- [~] Identical edit deduplication. (deferred → see plans/v0.0.3/)
- [~] Overlapping edit rejection with both rule IDs in the diagnostic. (deferred → see plans/v0.0.3/)
- [~] Stale file hash and stale expected-text rejection. (deferred → see plans/v0.0.3/)
- [~] UTF-8 byte ranges and CRLF input. (deferred → see plans/v0.0.3/)
- [~] Formatter failure and parser failure with no write. (deferred → see plans/v0.0.3/)
- [~] Atomic-write failure injection with original file preserved. (deferred → see plans/v0.0.3/)
- [~] Read-only files, symlinks, and path traversal outside scan roots. (deferred → see plans/v0.0.3/)
- [~] `--fix-dry-run` byte-for-byte matches the subsequent applied diff. (deferred → see plans/v0.0.3/)
- [~] Filters, ignores, baseline incompatibility, and BP enable/disable behavior. (deferred → see plans/v0.0.3/)

### 4.3 Performance and release checks

- [~] Benchmark normal scans before and after edit payload support; do not build
  replacements unless fix mode is requested. (deferred → see plans/v0.0.3/)
- [~] Benchmark fix planning and application on files with many findings. (deferred → see plans/v0.0.3/)
- [~] Run `cargo fmt --all -- --check`. (deferred → see plans/v0.0.3/)
- [~] Run focused edit-engine and Go detector tests. (deferred → see plans/v0.0.3/)
- [~] Run `cargo test --all-features`. (deferred → see plans/v0.0.3/)
- [~] Run repository lint checks. (deferred → see plans/v0.0.3/)
- [~] Run a real Go project dry-run and inspect every proposed diff. (deferred → see plans/v0.0.3/)
- [~] Document `--fix`, `--fix-dry-run`, safety guarantees, skipped-edit
  reasons, and the review-required boundary. (deferred → see plans/v0.0.3/)
