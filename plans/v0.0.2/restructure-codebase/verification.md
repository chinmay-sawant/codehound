...
Master verification procedure for the v0.0.2 file-split plan. Run per-phase and per-batch verifications to catch regressions early. The canary tests in each phase catch the most common split mistakes (missing `pub use` re-exports, broken `..` paths, forgotten `cargo:rerun-if-changed`).
...
- [x] `CHANGELOG.md` is updated with a v0.0.2 entry summarising the file-split refactor
