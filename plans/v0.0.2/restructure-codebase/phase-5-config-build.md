...
> **Status:** **Complete.** All 5 active splits done (§5.2 build.rs, §5.3 cwe/registry.toml, §5.4 perf/registry.toml, §5.6 ci.yml) + §5.1 Cargo.toml no-split + §5.5 schema.json skipped — no source files have been moved yet.
...
- [x] Confirm: Cargo's manifest format intentionally does **not** support a split mechanism for `Cargo.toml`. The Cargo reference book explicitly states `Cargo.toml` must be a single file at the package root.
- [x] The `[features]` and `[lints.rust].check-cfg` blocks are tightly coupled by feature name; any "split" would either be impossible (features block) or massively disproportionate (workspace refactor).
- [x] **Recommendation: leave as-is.** The only realistic improvement is a stronger convention: keep `[features]` and `[lints.rust]` adjacent so a future maintainer adding a feature flag is reminded to add the same name to `check-cfg`.
...
- [x] Slim `build.rs` to `mod` decls + `fn main()`. Reads the 3 input files, runs the dedupe assertions, calls the writers (~1 500 chars)
- [x] Create `build/types.rs` with `RegistryFile`, `RegistryDetector`, `PerfRegistryFile`, `PerfRegistryDetector`, `JsonRule` (~1 200 chars)
- [x] Create `build/parse.rs` with `parse_rules`, `parse_rule_id`, `parse_cwe_number`, `parse_perf_number`, `build_cwe_rule_map`, `build_perf_rule_map` (~1 800 chars)
- [x] Create `build/escape.rs` with `escape_rust_string` (~600 chars)
- [x] Create `build/gen_catalogue.rs` with `generate_rule_catalogue_code` (~1 400 chars)
- [x] Create `build/gen_cwe.rs` with `generate_cwe_catalog_code`, `generate_go_metadata_code`, `generate_go_registry_code` (~3 800 chars)
- [x] Create `build/gen_perf.rs` with `generate_go_perf_metadata_code`, `generate_go_perf_registry_code` (~2 500 chars)

### New `build.rs` shape

- [x] Replace `build.rs` body with:
  ```rust
  mod types;
  mod parse;
  mod escape;
...
- [x] Generated code is byte-for-byte the same.
- [x] `OUT_DIR/...` filenames do not change.
- [x] The six `include!(concat!(env!("OUT_DIR"), "/..."))` sites continue to work without edits.
- [x] `JsonRule` is shared by `parse.rs` and the three generator files; re-export via `use super::types::JsonRule;`.
- [x] No `Cargo.toml` changes — `[build-dependencies]` does not need to grow.
...
- [x] Create `cwe/registry.general_security.toml` (75 entries)
- [x] Create `cwe/registry.access_control.toml` (30 entries)
- [x] Create `cwe/registry.credentials_and_secrets.toml` (15 entries)
- [x] Create `cwe/registry.information_exposure.toml` (12 entries)
- [x] Create `cwe/registry.cryptography.toml` (9 entries)
- [x] Create `cwe/registry.injection.toml` (7 entries)
- [x] Create `cwe/registry.configuration.toml` (6 entries)
- [x] Create `cwe/registry.concurrency.toml` (6 entries)
- [x] Create `cwe/registry.input_validation.toml` (5 entries)
- [x] Create `cwe/registry.deserialization.toml` (3 entries)
- [x] Create `cwe/registry.request_handling.toml` (2 entries)
- [x] Create `cwe/registry.input_validation_redos.toml` (2 entries)
- [x] Create `cwe/registry.path_traversal.toml` (1 entry)
- [x] Create `cwe/registry.network_binding.toml` (1 entry)
- [x] Create `cwe/registry.file_handling.toml` (1 entry)
- [x] Either keep the existing `cwe/registry.toml` as an **empty placeholder** (the `build.rs` change below handles the directory glob), or delete it altogether and add a `cwe/registry/` directory.

### `build.rs` glue (do this as part of §5.2 if not already)

- [x] Replace the single `read_to_string` with a directory walk (`fs::read_dir("src/lang/go/detectors/cwe/registry")` filtered to `*.toml`), `toml::from_str` each, and `extend` the `Vec<RegistryDetector>`.
- [x] Update `cargo:rerun-if-changed` from one file to a directory.

### Compatibility notes

- [x] The 15 per-domain files are fully independent — the `cwe` field is a global `u32` and must remain unique across files. The `build.rs` dedupe assertion (`assert_eq!(supported_ids.len(), cwe_registry.detector.len(), "duplicate CWE ids in registry.toml")`) still works after the merge step.
- [x] All 175 detectors are still emitted to `go_cwe_registry.rs` and `go_cwe_metadata.rs` with identical content.
- [x] **No test changes** — no test opens `cwe/registry.toml` by path.
- [x] **No Rust source changes** beyond `build.rs`. The six `include!(...)` sites continue to read the same `OUT_DIR` files.

### Caveat

- [x] The `general_security` slice (75 entries, ~6 KB) is itself a candidate for a second-order split by CWE numeric range (e.g. `general_security_1xx.toml`, `general_security_2xx.toml`). Only do this if `general_security` itself becomes hard to navigate.
...
- [x] Create `perf/registry.general_perf.toml` (85 entries)
- [x] Create `perf/registry.gin_framework.toml` (20 entries)
- [x] Create `perf/registry.data_access.toml` (20 entries)
- [x] Create `perf/registry.protocols.toml` (10 entries)
- [x] Create `perf/registry.request_path.toml` (9 entries)
- [x] Create `perf/registry.parsing_in_loops.toml` (8 entries)
- [x] Create `perf/registry.loop_allocations.toml` (8 entries)

### `build.rs` glue (do this as part of §5.2 if not already)

- [x] Same as §5.3 — directory walk instead of single `read_to_string`.

### Test update required (this is the only Phase 5 test change)

- [x] `tests/go_perf_registry_generation.rs:7` reads `std::fs::read_to_string("src/lang/go/detectors/perf/registry.toml")` directly. After the split, replace the single `read_to_string` with a `read_dir` loop, concatenate the per-domain content, and continue with the same `perf = N` line parsing.

### Compatibility notes

- [x] All 160 detectors are still emitted to `go_perf_registry.rs` and `go_perf_metadata.rs` with identical content.
- [x] The `general_perf` slice (85 entries, ~6.6 KB) is the largest and may itself benefit from a second split by PERF numeric range.
...
- [x] Create `.github/actions/rust-toolchain-cache/action.yml` (composite) — common 4 steps: `actions/checkout@v4`, `dtolnay/rust-toolchain@stable` (with optional `toolchain` + `components` inputs), `actions/cache@v4` keyed on `runner.os + matrix/features + Cargo.lock`. Removes ~12 lines of duplicate steps from `test`, `lint`, `bench`.
- [x] Slim `.github/workflows/ci.yml` to `name`, `on`, `env`, four job `uses:` blocks (or `needs:` chains into reusable workflows). Single source of truth for checkout + toolchain + cache.
- [x] Create `.github/workflows/test.yml` (reusable, `workflow_call`) — the matrix test job body. Lets the perf/bench workflow reuse the same test job.
- [x] Create `.github/workflows/lint.yml` (reusable, `workflow_call`) — the lint job body (rustfmt, clippy). Reuse for PR-lint workflow if added later.
- [x] Create `.github/workflows/msrv.yml` (reusable, `workflow_call`) — the MSRV job body. Reuse.
- [x] Create `.github/workflows/bench.yml` (reusable, `workflow_call`) — the bench + perf-budget shell block. Could also be triggered on `workflow_dispatch` for ad-hoc runs.
- [x] Create `scripts/check_bench_budget.sh` (extracted from the `bench` job's perf-budget check) — cleaner than a composite action (composite actions are best for steps, not for shell logic).

### Compatibility notes

- [x] **Branch triggers stay in `ci.yml`.** Reusable workflows that are also `workflow_call`-only do **not** trigger on `push`/`pull_request` by themselves; they must be invoked. The branch triggers therefore must remain on the root `ci.yml`.
- [x] The `env:` block at workflow level is **not** inherited by `workflow_call` jobs. Re-declare on each reusable workflow.
- [x] The current cache key `${{ runner.os }}-cargo-${{ matrix.features }}-${{ hashFiles('**/Cargo.lock') }}` becomes an input to the composite action. The existing CI cache will be invalidated once on first run with the new key.

### Caveat

- [x] Reusable workflows add a layer of indirection; for a 4-job workflow they are sometimes heavier than they are worth. The **composite-action path is the better first move**. After moving checkout + toolchain + cache into a composite action, the root `ci.yml` shrinks to ~1.5 KB.
...
- [x] **§5.2 `build.rs`** — split first. Makes the §5.3 / §5.4 changes easier because the new `build/parse.rs` and `build/gen_*.rs` modules absorb the "merge per-domain registries" logic.
...
- [x] **1. Split `build.rs`** into a `build/` directory of focused modules — **highest leverage**, lowest risk.
...
- [x] **The `build.rs` split is independent of the registry splits.** If you split only `build.rs`, the registry TOML files can stay monolithic and `build.rs` reads them as today. If you split the registries, `build.rs` must grow a directory-read step regardless of whether `build.rs` itself is also modularized.
- [x] **CI cache key impact:** any change to the registry file path invalidates the cargo cache in CI on the first run after the merge. This is a one-time cost.
- [x] **External rule-pack loading** (mentioned in `plans/p2-implementation/missing-D-rule-pack-extensibility.md`) is a future direction that will require a *runtime* registry loader, separate from the build-time registry. The proposed split (per-domain TOML files, mirrored under `domains/`) is the right precondition for that future change.

---

## Phase 5 verification

- [x] After every batch: `cargo build --features go,python`
- [x] Final, after all config & build splits: `cargo clean && cargo build --features go,python`
- [x] Confirm generated `OUT_DIR/*.rs` is byte-identical (see `verification.md` § "Phase 5 (Config & build)" for the diff procedure).
- [x] The CI composite action is verified on the first PR; existing CI cache is invalidated once.
