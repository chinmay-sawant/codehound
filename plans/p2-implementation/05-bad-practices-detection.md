...
- **`--bp-only` is implemented** (in `src/cli/args.rs:52-53`, wired in `args_impl.rs:22-25`).
...
- [x] `check_no_prod_expect.sh` wired in CI (`.github/workflows/ci.yml:58`); production `.expect()` calls cleaned up from walker/registry
