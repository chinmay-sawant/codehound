...
- [ ] Remove the 112 nested `PERF-101`…`PERF-212` objects incorrectly stored as
  extra fields inside the top-level `PERF-100` object.
...
- [ ] Fix semantic mismatches before attaching edits. Known example:
  ...
  PERF-102 as HTTP transport sharing.
- [ ] Require each registered detector's fixture to assert the JSON rule name
  and intended pattern, not only the emitted ID.
- [ ] Verify all 175 CWE registry entries against their JSON descriptions.
- [ ] Move the 13 shipped BP definitions to the planned
  `ruleset/golang/bad-practices.json` before making BP fix metadata generated.
...
- [ ] Add a schema/coverage test that fails on extra fields, mixed ID formats,
  ...
  detector/message that does not match its rule definition.
- [ ] Run the registry check in `cargo test` and in CI.
...
- [ ] Keep `RuleMetadata.fix` and `Finding.fix` as human-readable remediation.
...
- [ ] Emit an edit only from an exact AST match. A detector's line/column,
  source-window heuristic, or metadata text is insufficient.
- [ ] Do not reuse the currently dormant `Finding.byte_offset` /
  ...
  replacement. Prefer an explicit `SourceEdit`.
- [ ] Keep fixes optional so reporters, caches, and non-Go detectors remain
  backward-compatible.

### 1.2 Define the CLI contract

- [ ] Add `--fix`: apply only `MachineApplicable` edits.
- [ ] Add `--fix-dry-run`: print the exact proposed diff and write nothing.
- [ ] Honor `--only`, `--skip`, `--bp-only`, `--no-bp`, path filters, ignored
  findings, and configuration severity/rule filters before planning edits.
- [ ] Make fix modes incompatible with `--baseline`, `--list-rules`,
  `--explain`, and cache-pruning commands.
- [ ] Force a fresh uncached scan in V2.0.0 fix mode. This is smaller and safer
  than changing the cache schema before the edit model stabilizes.
- [ ] Report files changed, edits applied, edits skipped, conflicts, formatter
  failures, and findings remaining after the verification scan.
- [ ] Compute the process exit code from the post-fix scan; return non-zero for
  edit conflicts or validation/write failures.
...
- [ ] Sort accepted edits by descending byte offset so earlier replacements do
  not invalidate later offsets.
- [ ] Apply all edits in memory.
- [ ] Run `gofmt` on the in-memory result through stdin; do not require
  `goimports` or add a dependency.
- [ ] Reparse the formatted result with the existing Go tree-sitter parser.
- [ ] Reuse/generalize `engine::cache::io::write_atomic` for the final write
  instead of introducing a second atomic-write implementation.
...
- [ ] Detect aliases, dot imports, blank imports, local identifiers that shadow
  package names, and multiple import blocks.
- [ ] Decline the edit when import resolution is ambiguous.
...
- [ ] **PERF-001 — Regular Expression Compilation Inside Loop:** hoist only a
  valid constant `regexp.MustCompile` pattern to a collision-free binding.
- [ ] **PERF-016 — Buffer Reallocation Inside Loop:** hoist a non-escaping
  `bytes.Buffer` and call `Reset`; refuse aliases, copies, or retained results.
- [ ] **PERF-024 — Crypto Hashing In Tight Loop:** reuse a non-escaping
  fixed-algorithm hasher with `Reset`; exclude keyed/dynamic HMAC construction.
...
- [ ] **PERF-109 — Map Key Recomputed In Loop Without Caching:** hoist a pure,
  loop-invariant, non-panicking key expression.
...
- [ ] **PERF-153 — http.Cookie.String Called Repeatedly:** cache the string only
  when the cookie and all aliases are not mutated.
- [ ] **PERF-179 — strings.Replacer Not Used For Repeated Replace:** hoist a
  replacer for loop-invariant replace-all operands.
- [ ] **PERF-192 — Map Without Size Hint:** add `len(src)` only for a proven
  one-to-one population loop with a known source collection.
- [ ] **PERF-203 — net.IP.String Repeated In Hot Path:** cache the result only
  when the IP slice and aliases are not mutated.
...
- [ ] **PERF-103 — HTTP Response Body Not Closed:** insert
  ...
  guard when response ownership does not transfer.
...
- [ ] **PERF-114 — Manual Loop Copy Instead Of copy() Builtin:** replace only the
  canonical side-effect-free full-copy loop with proven compatible bounds.
- [ ] **PERF-119 — Multiple Separate Appends Instead Of Spread Concatenation:**
  ...
  not observe or alias the destination slice.
...
- [ ] **PERF-122 — HasPrefix Followed By Slice Instead Of TrimPrefix:** collapse
  an exact one-statement/no-else guard with stable expressions.
- [ ] **PERF-123 — Redundant make Argument With Zero Value:** remove zero only
  from proven map/channel forms; never rewrite `make([]T, 0, cap)`.
- [ ] **PERF-128 — Multiple Independent Appends Can Be Combined:** use the same
  purity/alias gates as PERF-119 for three or more appends.
- [ ] **PERF-129 — Range Loop Copies Value When Only Index Needed:** remove only
  an explicit blank value binding (`for i, _ := range`).
- [ ] **PERF-130 — Unnecessary Function Wrapper Adding Call Overhead:** inline
  ...
  call; exclude `go`, `defer`, return values, locals, and control flow.
- [ ] **PERF-133 — sort.Slice Closure Allocation Inside Loop:** replace only an
  exact ascending comparator over a proven basic slice type.
- [ ] **PERF-158 — Sorting Slice Of Basic Types With Closure:** share the
  PERF-133 fixer; explicitly gate float/NaN behavior and Go-version API choice.
- [ ] **PERF-167 — WaitGroup.Add Inside Goroutine:** move an unconditional
  positive-constant `Add` immediately before its matching goroutine.
- [ ] **PERF-173 — time.Tick Not Stopped Causing Goroutine Leak:** introduce a
  function-scoped `NewTicker`, use `.C`, and insert `defer Stop`.
...
- [ ] **Loop allocation/reuse proposals:** PERF-002, PERF-003, PERF-004,
  ...
  freshness, error, capacity, Go-version, or user-limit decisions.
...
- [ ] **Database/framework proposals:** PERF-071, PERF-072, PERF-073, PERF-074,
  ...
  query, transaction, hook, batch, framework lifetime, or cache policy.
...
- [ ] **BP-2:** wrap `return err` with inferred operation context only after
  reviewing error text/identity compatibility.
...
- [ ] **BP-10:** replace `time.After` loops with a correctly stopped, drained,
  and reset timer after reviewing lifecycle behavior.
- [ ] **BP-11:** scope per-iteration defers in a closure/helper only when
  `break`, `continue`, `return`, panic, and error semantics are preserved.
...
- [ ] Keep the existing vulnerable fixture and safe fixture for detection.
- [ ] Add exact transformed-source golden coverage.
- [ ] Add one refusal fixture for every safety precondition.
- [ ] Assert the transformed source parses and is already `gofmt`-clean.
- [ ] Assert the originating finding disappears on rescan.
- [ ] Assert a second fix run produces zero edits.
- [ ] Compile/run a temporary Go module where the rule's dependencies permit
  it; do not claim semantic safety from string snapshots alone.

### 4.2 Edit-engine tests

- [ ] Multiple non-overlapping edits in one file.
- [ ] Identical edit deduplication.
- [ ] Overlapping edit rejection with both rule IDs in the diagnostic.
- [ ] Stale file hash and stale expected-text rejection.
- [ ] UTF-8 byte ranges and CRLF input.
- [ ] Formatter failure and parser failure with no write.
- [ ] Atomic-write failure injection with original file preserved.
- [ ] Read-only files, symlinks, and path traversal outside scan roots.
- [ ] `--fix-dry-run` byte-for-byte matches the subsequent applied diff.
- [ ] Filters, ignores, baseline incompatibility, and BP enable/disable behavior.

### 4.3 Performance and release checks

- [ ] Benchmark normal scans before and after edit payload support; do not build
  replacements unless fix mode is requested.
- [ ] Benchmark fix planning and application on files with many findings.
- [ ] Run `cargo fmt --all -- --check`.
- [ ] Run focused edit-engine and Go detector tests.
- [ ] Run `cargo test --all-features`.
- [ ] Run repository lint checks.
- [ ] Run a real Go project dry-run and inspect every proposed diff.
- [ ] Document `--fix`, `--fix-dry-run`, safety guarantees, skipped-edit
  reasons, and the review-required boundary.
