...
- [ ] Create `tests/go_bad_practice_integration.rs` with fixture-driven tests that mirror `tests/go_perf_detector_integration.rs`:
  ```rust
  #[test]
  ...
  ```
- [ ] Create a `tests/helpers/go_bp_cases.rs` helper for fixture discovery (analogous to `tests/helpers/go_perf_cases.rs`)
- [ ] Remove the BP-specific checks from generic tests (`tests/engine_config_cli_filters.rs`, `tests/reporting_json_finding.rs`) that currently serve as BP integration — make those config-filter tests generic only, and put BP-specific tests in the dedicated file
...
- [ ] Add a fixture where the recursive `sync.Once.Do()` call is in a **separate function** (the closure calls a named function that calls `.Do()` again) — this is the harder case because the detector must walk up the call chain
- [ ] BP-15 currently only tests a direct recursive closure; the separate-function case may not be detected
...
- [ ] Add `BadPracticeCategory` enum in `src/rules/mod.rs` or a new `src/rules/bp_category.rs`:
  ```rust
...
- [ ] Use the enum instead of `category_for_rule_id()` string matching in `src/rules/category.rs`
- [ ] Ensure JSON/SARIF/text reporters continue to emit `"category": "bad_practice"` (the enum is for internal dispatch, not output)

### 2.2 Create `ruleset/golang/bad-practices.json`

- [ ] Create `ruleset/golang/bad-practices.json` with the same structure as `golang.json`:
...
- [ ] Define a `BadPracticeRuleMetadata` struct in `src/rules/` (analogous to `CweRef` in `src/cwe/reference.rs`)

### 2.3 Codegen from `bad-practices.json`

- [ ] Add a `gen_bp.rs` in `build/` that generates `META_BP_N` constants from `ruleset/golang/bad-practices.json`
- [ ] Update `build.rs` to invoke the BP codegen
- [ ] Replace inline constants in `metadata.rs` with `include!` of the generated code

### 2.4 Verify `--list-rules` and `--explain` for BP

- [ ] `--list-rules` should show BP rules with proper category grouping
- [ ] `--explain BP-1` should pull from the generated metadata (same as CWE/PERF)
- [ ] Remove the inline dispatch table `metadata_for()` if the generated metadata makes it redundant
...
- [ ] **BP-16**: `time.Sleep` in test — detect `time.Sleep` (not in a retry loop). Use `testing.Short()` or a ticker instead.
  - Detection: AST walk for `time.Sleep()` in `*_test.go` files, outside a `for`/`range` retry attempt
- [ ] **BP-17**: `t.Error` followed by `t.Fatal` — redundant, should use `t.Errorf` then `return`.
  - Detection: line scan for consecutive `t.Error` + `t.Fatal` calls
- [ ] **BP-18**: `t.Error`/`t.Errorf` without `t.FailNow`/`return` — test continues after error.
...
- [ ] **BP-20**: Table-driven test without `t.Run` — all cases in single subtest.
...
- [ ] **BP-22**: TestMain without `os.Exit` — if `TestMain` is defined but doesn't call `os.Exit(m.Run())`.
  - Detection: AST scan for `func TestMain(m *testing.M)` body missing `os.Exit`
- [ ] **BP-23**: `testing.Short()` not checked — detect tests that take >100ms without checking `testing.Short()`
  - Detection (heuristic): function with `t.Run` body > 20 lines and no `testing.Short()` guard
- [ ] **BP-24**: Test file without any test functions — file is `*_test.go` but has zero `func Test*` declarations.
  - Detection: file-level scan of `*_test.go` files
- [ ] **BP-25**: Test helper returns error instead of calling `t.Fatal` — helper returns `error` when `t.Fatal` would be simpler.
...
- [ ] Add BP-16..BP-25 detection functions to a new `rules/testing.rs` file
- [ ] Use `unit.path.ends_with("_test.go")` or file-name check in the `SourceIndex` pre-filter
- [ ] Register in `BAD_PRACTICE_RULES` in `dispatch.rs`
- [ ] Add metadata constants for BP-16..BP-25 in `metadata.rs` (or generated from codegen)

### 3.3 Test fixtures

- [ ] Create 20 fixture files (10 vulnerable + 10 safe) in `tests/fixtures/go/bad_practices/`
- [ ] Safe fixtures should use the correct pattern
- [ ] Vulnerable fixtures should use `_test.go` suffix
- [ ] Register in `tests/fixtures/manifest.toml`
- [ ] Verify via the dedicated `tests/go_bad_practice_integration.rs`
...
- [ ] **BP-26**: Context not first parameter in function signature
...
- [ ] **BP-28**: Interface with a single method — should be a function type instead
  - Detection: `interface_type` with exactly one method
- [ ] **BP-29**: Interface with excessive methods (>5) — "interface bloat"
...
- [ ] **BP-38**: Unexported helper with no internal callers within the package
...
- [ ] **BP-40**: Package-level var/const block not used for related declarations
...
- [ ] **BP-43**: Dot import (`import . "pkg"`) used outside test files
  - Detection: line scan for `import . "` with file not ending in `_test.go`
- [ ] **BP-44**: Blank import (`import _ "pkg"`) without `init()` or driver justification
...
- [ ] **BP-45**: Receiver name inconsistent across methods of the same type
  - Detection: methods with receiver `(t *T)` and `(this *T)` on the same type — inconsistent naming

### 4.3 Detection approach

- [ ] Add BP-26..BP-45 detection functions to new files:
...
- [ ] Register in `BAD_PRACTICE_RULES` in `dispatch.rs`
- [ ] Add metadata constants for BP-26..BP-45 in `metadata.rs`
...
- [ ] **BP-60**: Test dependency in main `go.mod` (not in `go.test.mod` or `tools.go`)
  - Detection: check test-only packages in `require` block vs only imported in `*_test.go` files
- [ ] **BP-61**: Indirect dependency not listed in `go.mod` (Go 1.17+ requires `// indirect` comments)
...
- [ ] **BP-63**: Dependency with known security advisory (CVE) not updated
  - Detection: integrate with `go list -json -m -u all` advisory data or `golang.org/x/vuln`
- [ ] **BP-64**: Replace directive in `go.mod` pointing to local filesystem
...
- [ ] Add BP-46..BP-65 detection functions to new files:
  - `rules/production_hardening.rs` (BP-46..BP-55)
  - `rules/dependency_hygiene.rs` (BP-56..BP-65)
- [ ] BP-56..BP-65 need `go.mod` parsing — integrate with existing `src/engine/dependencies/` infrastructure
- [ ] BP-57, BP-58, BP-61, BP-64, BP-65 are project-level scans (not per-file)
- [ ] Register in `BAD_PRACTICE_RULES` in `dispatch.rs` (per-file rules) or add a new project-level BP detector for project-wide rules
- [ ] Add metadata constants for BP-46..BP-65 in `metadata.rs`
...
- [ ] **BP-12**: Unbuffered channel send from multiple goroutines without adequate receivers — needs taint to track goroutine lifecycle
- [ ] **BP-14**: Goroutine without `ctx.Done` select — needs taint to track if a cancellable context reaches the goroutine
...
- [ ] Create `docs/bad-practices.md` — one paragraph per BP rule with rationale and canonical fix. Source: `plans/v2.0.0/antipattern-remediation/bad-practices-scope.md` (also tracked in `05-cross-cutting-remaining.md`)
