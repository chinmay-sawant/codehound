...
pub(crate) fn detect_cwe_353(unit: &ParsedUnit, facts: &GoUnitFacts, out: &mut Vec<Finding>) {
    ...
    let ingests_body = facts.source_index.has("io.ReadAll(")
        && facts
            .source_index
            .has_any(&["INSERT INTO telemetry", "INSERT INTO agent_reports"]);
    if !ingests_body {
        return;
    }
    if facts
        .source_index
        .has_any(&["X-Body-Mac", "ConstantTimeCompare(expected, got)"])
    {
        return;
    }
...
pub(crate) fn detect_cwe_356(unit: &ParsedUnit, facts: &GoUnitFacts, out: &mut Vec<Finding>) {
    ...
    let destructive_delete = (facts.source_index.has("func PurgeTenant(")
        ...
        || (facts.source_index.has("func DeleteWorkspaceRecords(")
            && facts
                .source_index
                .has("DELETE FROM workspaces WHERE slug = ?"));
    if !destructive_delete {
        return;
    }
    if facts
        .source_index
        .has_any(&["X-Confirm-Purge", "X-Confirm-Delete"])
    {
        return;
    }
...
pub(crate) fn detect_cwe_494(unit: &ParsedUnit, facts: &GoUnitFacts, out: &mut Vec<Finding>) {
    ...
    let downloads_bundle =
        facts.source_index.has("http.Get(") && facts.source_index.has("/tmp/worker.bin");
    if !downloads_bundle {
        return;
    }
    if facts
        .source_index
        .has_any(&["sha256.Sum256(", "integrity check failed"])
    {
        return;
    }
...
pub(crate) fn detect_cwe_924(unit: &ParsedUnit, facts: &GoUnitFacts, out: &mut Vec<Finding>) {
    ...
    let applies_payment_webhook = (facts.source_index.has_any(&[
        "AcceptWebhook(",
        "AcceptWebhookPure(",
    ...
    if facts
        .source_index
        .has_any(&["X-Signature", "hmac.New(sha256.New", "hmac.Equal("])
    {
        return;
    }
